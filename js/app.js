(function () {
  ...
  // "off" -> lanjut berurutan, berhenti di lagu terakhir
  // "all" -> lanjut berurutan, ulang dari lagu pertama setelah lagu terakhir
  // "one" -> ulang lagu yang sama terus-menerus
  let repeatMode = "off";
  let isShuffle = false;
  ...
  nextBtn.addEventListener("click", () => {
    if (activeTrackIndex === -1) return;
    // tombol next selalu lanjut (manual skip), beda dari auto-lanjut saat lagu selesai
    const idx = getShuffleOrSequentialIndex(true);
    loadTrack(idx, true);
  });
  ...
  function randomIndexExcluding(excludeIndex) {
  ...
  // menghitung index track berikutnya. wrapAlways = true untuk tombol manual (selalu lanjut/wrap)
  function getShuffleOrSequentialIndex(wrapAlways) {
    if (isShuffle) return randomIndexExcluding(activeTrackIndex);
    const seqNext = activeTrackIndex + 1;
    if (seqNext < TRACKS.length) return seqNext;
    return wrapAlways || repeatMode === "all" ? 0 : null;
  }

  /* ---- REPEAT ---- */
  const repeatIcons = { off: "🔁", all: "🔁", one: "🔂" };
  const repeatLabels = { off: "mati", all: "semua lagu", one: "satu lagu" };
  ...
  repeatBtn.addEventListener("click", () => {
    ...
    repeatBtn.setAttribute("aria-label", `Ulangi: ${repeatLabels[repeatMode]}`);
  });
  ...
  audioEl.addEventListener("ended", () => {
    ...
    const nextIndex = getShuffleOrSequentialIndex(false);
    if (nextIndex === null) {
      // sudah lagu terakhir & repeat mati & shuffle mati -> berhenti
      pauseAudio();
      return;
    }
